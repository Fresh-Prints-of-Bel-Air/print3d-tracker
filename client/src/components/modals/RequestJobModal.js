...
const RequestJobModal = ({ job: { lastCreatedJobID }, user: { user }, addJob, updateUser }) => {
    ...
    const handleQuantityChange = (e) => { 
        ...
        copyArray[e.target.name].extras = 0; //extra parts that can be built beyond the amount requested 
    ...
    const formSubmit = (e) => { 
        ...
        console.log(jobForm);
        addJob({ ...jobForm, requester: user.name, requesterId: user._id, status: "Requested", dateRequested: new Date().toISOString().split('T')[0] }, {...user, lastJobRequest: jobForm}); 
